...
// Extract JS
// Need to be careful because there's also Tailwind CDN script tag at the top.
// The main script tag is at the bottom right before </body>.
// So let's find <script> that is just <script> without src.
const scriptRegex = /<script>\s*([\s\S]*?)<\/script>\s*<\/body>/;
